...
// --- Organisation Name ---
function loadOrgName(){return localStorage.getItem(PK+'orgName')||''}
function saveOrgName(name){localStorage.setItem(PK+'orgName',name)}
...
function getDefaultStaff(){return{
  ...
  housekeeping:[
    {name:'MANJULA',role:'housekeeping',active:true,nightPref:[1]},
    {name:'ESHWARI',role:'housekeeping',active:true,nightPref:[2]},
    {name:'MALLAMMA',role:'housekeeping',active:true,nightPref:[3]},
...
function loadStaff(){
  var r=localStorage.getItem(PK+'staff');
  if(r){
    var d=JSON.parse(r);
...
function saveStaff(d){localStorage.setItem(PK+'staff',JSON.stringify(d))}
function loadRota(y,m,tab){return JSON.parse(localStorage.getItem(PK+'rota_'+tab+'_'+y+'_'+m)||'null')}
function saveRota(y,m,tab,rota){localStorage.setItem(PK+'rota_'+tab+'_'+y+'_'+m,JSON.stringify(rota))}
function loadEdits(y,m,tab){return JSON.parse(localStorage.getItem(PK+'edits_'+tab+'_'+y+'_'+m)||'null')}
function saveEdits(y,m,tab,edits){localStorage.setItem(PK+'edits_'+tab+'_'+y+'_'+m,JSON.stringify(edits))}
...
function render(){
  ...
  if(!rota){
    html+='<div style="text-align:center;padding:3rem;color:var(--text-light)"><p style="font-size:1.1rem;margin-bottom:.5rem">No rota generated yet</p><p>Click <b>Generate</b> to create a balanced schedule, or click <b>Manual</b> to create a schedule manually.</p></div>';
    document.getElementById('rota-content').innerHTML=html;updateTabs();updateButtons();return;
  }
  ...
  // Re-apply any active highlight so a selection survives the table rebuild (item 3 fix).
...
// re-render — so the cell being typed into keeps focus/cursor during manual entry (items 3 & 4).
function refreshSummaryAndCoverage(){
  var y=state.currentYear,m=state.currentMonth,tab=state.currentTab;
  var days=getDays(y,m),rota=loadRota(y,m,tab),staff=loadStaff();
  if(!rota)return;
  var team=(staff[tab]||[]).filter(function(s){return s.active});
  ...
  // --- Day-header coverage + fully-staffed asterisk ---
  var table=document.querySelector('.rota-table');
  if(table){
    var headerCells=table.querySelectorAll('thead th'); // [0]=corner, [d]=day d
    for(var d=1;d<=days;d++){
      var th=headerCells[d];if(!th)continue;
      var cov={M:0,A:0,N:0},hasOff=false;
      nur.forEach(function(n){
        var s=rota[n.name]?rota[n.name][d-1]:'';
        if(s==='M')cov.M++;else if(s==='A')cov.A++;else if(s==='N')cov.N++;
        else if(s==='MA'){cov.M++;cov.A++}else if(s==='AN'){cov.A++;cov.N++}
  ...
  if(summary){
    ...
    team.forEach(function(p,ri){
      ...
      var cells=row.querySelectorAll('td'); // [0]name [1]G [2]M [3]MA [4]A [5]AN [6]N [7]O [8]PL [9]Total
      if(cells[1])cells[1].textContent=c.G;
      if(cells[2])cells[2].textContent=c.M;
      if(cells[3])cells[3].textContent=c.MA;
      if(cells[4])cells[4].textContent=c.A;
      if(cells[5])cells[5].textContent=c.AN;
      if(cells[6])cells[6].textContent=c.N;
      if(cells[7])cells[7].textContent=c.O;
      if(cells[8])cells[8].textContent=c.PL;
      // Total = live sum of all shift types (equals days-in-month when fully filled).
      if(cells[9])cells[9].textContent=c.G+c.M+c.MA+c.A+c.AN+c.N+c.O+c.PL;
    });
  }
}
function updateTabs(){
...
function changeMonth(v){
  if(v==='__custom__'){
    // Reset the dropdown back to the real current selection, then open the picker.
    var sel=document.getElementById('month-select');if(sel)sel.value=state.currentYear+'-'+state.currentMonth;
    showCustomDate();
    return;
  }
  var p=v.split('-');state.currentYear=+p[0];state.currentMonth=+p[1];undoStack=[];hasUnsaved=false;manualMode=false;setManualHint(false);buildMonthSelect();render();
}

// Custom date picker: jump to ANY month/year (past or future) outside the default list.
function showCustomDate(){
  var now=new Date();
  var monthOpts='';
  for(var mm=0;mm<12;mm++){monthOpts+='<option value="'+mm+'"'+(mm===state.currentMonth?' selected':'')+'>'+MONTH_NAMES[mm]+'</option>'}
  var yearOpts='';
  for(var yy=now.getFullYear()-5;yy<=now.getFullYear()+5;yy++){yearOpts+='<option value="'+yy+'"'+(yy===state.currentYear?' selected':'')+'>'+yy+'</option>'}
...
function applyCustomDate(){
  var mm=+document.getElementById('cd-month').value;
  var yy=+document.getElementById('cd-year').value;
  state.currentMonth=mm;state.currentYear=yy;
  undoStack=[];hasUnsaved=false;manualMode=false;setManualHint(false);
  closeModal();
  buildMonthSelect();
  render();
}
function switchTab(t){state.currentTab=t;undoStack=[];hasUnsaved=false;manualMode=false;setManualHint(false);render()}
function generateRota(){
  manualMode=false;setManualHint(false);
...
// ======= MANUAL MODE (spreadsheet-style hand entry) =======
var manualMode=false;
var VALID_SHIFTS=['M','G','A','N','O','PL','MA','AN'];

function setManualHint(on){var el=document.getElementById('manual-hint');if(el)el.classList.toggle('show',!!on)}

// Build a blank rota for the month. G-shift person is auto-filled (G weekdays, O Sundays);
// all other staff cells are left blank for manual entry.
function buildBlankRota(y,m,tab){
  var staff=loadStaff(),team=staff[tab]||[];
  if(!team.filter(function(s){return s.active}).length)return null;
  var days=getDays(y,m);
  var rota={};
  team.forEach(function(s){if(s.active)rota[s.name]=new Array(days).fill('')});
  var gPerson=team.find(function(s){return s.role==='g-shift'&&s.active});
...
function manualRota(){
  var staff=loadStaff();
  var team=(staff[state.currentTab]||[]).filter(function(s){return s.active});
  if(!team.length){showToast('No staff configured — open Staff Setup first');return}
  var existing=loadRota(state.currentYear,state.currentMonth,state.currentTab);
  var start=function(){
    var blank=buildBlankRota(state.currentYear,state.currentMonth,state.currentTab);
    if(!blank){showToast('No staff configured — open Staff Setup first');return}
    undoStack=[];hasUnsaved=false;
    saveRota(state.currentYear,state.currentMonth,state.currentTab,blank);
    // A fresh manual rota has no auto-generated content, so clear any edit tracking.
    saveEdits(state.currentYear,state.currentMonth,state.currentTab,{});
    manualMode=true;setManualHint(true);
    render();
    showToast('Manual mode — type shifts directly into cells');
  };
  if(existing){
...
// Ordered list of editable nurse names (excludes G-shift) — used for arrow/Tab navigation.
var manualNav={names:[],days:0};

// Move focus to a specific editable cell (by nurse name + 0-indexed day). No wrap at edges.
function focusManualCell(name,di){
  var table=document.querySelector('.rota-table');if(!table)return false;
  var row=table.querySelector('tr[data-nurse="'+name+'"]');if(!row)return false;
  var cell=row.querySelector('td.shift-cell[data-day="'+di+'"]');if(!cell)return false;
  cell.focus();
  return true;
}

// Compute the next editable cell given a direction, skipping the (non-editable) G-shift row.
function manualNeighbour(name,di,dir){
  var names=manualNav.names,days=manualNav.days;
  var ni=names.indexOf(name);
  if(ni<0)return null;
  if(dir==='left'){return di>0?{name:name,di:di-1}:null}
  if(dir==='right'){return di<days-1?{name:name,di:di+1}:null}
  if(dir==='up'){return ni>0?{name:names[ni-1],di:di}:null}
  if(dir==='down'){return ni<names.length-1?{name:names[ni+1],di:di}:null}
  return null;
}
...
function wireManualCells(){
  ...
  table.classList.add('manual-mode');
  var staff=loadStaff();
  var team=(staff[state.currentTab]||[]).filter(function(s){return s.active});
  // In manual mode ALL active rows are editable — including the G-shift row (which is
...
// and surface non-blocking warnings (coverage / incompatible pairs).
function commitManualCell(name,di,cell){
  var raw=(cell.textContent||'').trim().toUpperCase();
  if(raw==='-')raw=''; // '-' is the placeholder for an empty cell, treat as blank
  var rota=loadRota(state.currentYear,state.currentMonth,state.currentTab);
  if(!rota||!rota[name]){return}
  var prev=rota[name][di]||'';
  // Empty is allowed (blank cell).
  if(raw===''){
    if(prev!==''){pushUndo();rota[name][di]='';saveRota(state.currentYear,state.currentMonth,state.currentTab,rota);markEdit(name,di);hasUnsaved=true;paintCell(cell,'');updateButtons();refreshSummaryAndCoverage()}
    else{paintCell(cell,'')}
    return;
  }
  // Reject invalid codes: flash red, revert to previous value.
  if(VALID_SHIFTS.indexOf(raw)===-1){
  ...
  if(raw===prev){paintCell(cell,raw);return}
  // Valid + changed: apply. Run validation for non-blocking warnings only.
  pushUndo();
  rota[name][di]=raw;
  ...
  paintCell(cell,raw);
  updateButtons();
  refreshSummaryAndCoverage(); // live Shift Count + coverage update (items 3 & 4)
  var errors=validate(rota,name,di,raw,staffForValidate(),state.currentTab);
  if(errors.length){showToast('Noted — '+errors[0]+(errors.length>1?' (+'+(errors.length-1)+' more)':''))}
}

function staffForValidate(){return loadStaff()}

// Repaint a cell to match a shift value (class + text) without a full re-render,
// so the cursor/flow isn't disrupted mid-entry. In manual mode a blank value is shown
// as an empty cell (not the '-' placeholder) for a clean spreadsheet feel.
function paintCell(cell,shift){
...
// Past/future months outside this range are reachable via the "Custom…" picker.
function buildMonthSelect(){
  var sel=document.getElementById('month-select');if(!sel)return;
  sel.innerHTML='';
  var now=new Date();
  var have=false;
  for(var i=0;i<=6;i++){
  ...
  // If the currently selected month is outside the default list (e.g. picked via Custom),
  // add it as an extra option so the dropdown reflects the real selection.
  var curVal=state.currentYear+'-'+state.currentMonth;
  for(var k=0;k<sel.options.length;k++){if(sel.options[k].value===curVal){have=true;break}}
  if(!have){
    var o2=document.createElement('option');
    o2.value=curVal;
    o2.textContent=MONTH_NAMES[state.currentMonth]+' '+state.currentYear+' (custom)';
    sel.insertBefore(o2,sel.firstChild);
  }
  // Last entry: the custom date picker (opens a month/year dialog).
  var oc=document.createElement('option');
  oc.value='__custom__';
  oc.textContent='Custom date…';
  sel.appendChild(oc);
  sel.value=curVal;
}
buildMonthSelect();
...
// Clear highlight when clicking outside the summary table — BUT keep it when the click
// is on the rota table itself (editing a cell under a selection must not reset visibility)
// or inside the inline edit popup. (Item 3 fix.)
document.addEventListener('click',function(e){
  var summary=document.getElementById('summary-tbl');
  if(!summary||!(hlState.names.length>0||hlState.shifts.length>0))return;
  if(summary.contains(e.target))return;              // clicking within the summary: handled by hl* toggles
  if(e.target.closest&&e.target.closest('.rota-table'))return; // editing/clicking the rota: keep highlight
  if(e.target.closest&&e.target.closest('#inline-popup'))return; // shift picker popup: keep highlight
  clearHighlight();
});
